    ...
    <div className="space-y-5">
      <section className="panel">
        ...
        <p className="mt-1 text-sm text-notion-secondary">
          {detailMode ? `当前路由: ${riskPaths.run(routeRunId)}` : ''}
        </p>
        {detailMode ? (
          <p className="mt-2 text-xs text-notion-secondary">
            <Link to={riskPaths.runsCompare} className="hover:underline">返回 Run 对比</Link>
          </p>
        ) : null}
      </section>
      ...
      <section className="panel">
        <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
          ...
          <Link
            to={
              selectedRunId
                ? riskPaths.learnerDetailRun(
                    selectedRunId,
                    selectedNodeId ? selectedNodeId : undefined,
                  )
                : riskPaths.learnerDetail
            }
