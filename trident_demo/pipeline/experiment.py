...
from trident_demo.core.tmagnifier import TMagnifier
from trident_demo.core.tscissors import TScissors
from trident_demo.core.tsieve import TSieve
from trident_demo.lib.utils import (
    has_year_prefix,
    infer_year_tag,
    is_benign_label,
...
class TridentStreamingExperiment:
    def __init__(self, cfg: Dict, logger, perf_recorder: Optional[Any] = None):
        self.cfg = cfg
        self.logger = logger
        ...
        self._perf_recorder = perf_recorder or PerformanceRecorder(
        ...
        self.benchmark_report_inputs: Dict[str, Any] = {}
        from trident_demo.export.live_flush import resolve_live_flush_settings
    ...
    def _write_live_run_status(self, status: str, *, rows_used: int = 0) -> None:
        if not self._live_flush_enabled():
            return
        from trident_demo.export.live_flush import write_live_run_status
    ...
    def _maybe_flush_live_qualification(self, data: pd.DataFrame) -> None:
        ...
        from trident_demo.export.live_flush import flush_qualification_artifacts
    ...
    def _flush_live_window_artifacts(self, time_series: list, entry: Dict[str, Any]) -> None:
        ...
        if bool(self._live_flush_settings.get("flush_window_csv", True)):
            from trident_demo.export.live_flush import flush_learner_count_csv
    ...
    def _run_decision_tree_analysis(
        ...
        *,
        ...
        from trident_demo.export.decision_tree_analysis import run_pipeline_decision_tree_analysis
        ...
        if bool(viz_cfg.get("enabled", True)):
            try:
                ...
                if self._perf_recorder.enabled:
                    ...
                    from trident_demo.export.dataset_topology import save_learner_network_topology
                    from trident_demo.benchmark.qualification import profile_learner_qualification
                    ...
                    if bool(viz_cfg.get("learner_topology_enabled", True)):
                        t_topo = perf_counter()
                    ...
                    if bool(viz_cfg.get("metric_audit_enabled", True)):
                        t_qual = perf_counter()
                    ...
                    from trident_demo.export.visualization import export_visualization_artifacts
            ...
            self.logger.info("Skip visualization artifact export (visualization.enabled=false)")
