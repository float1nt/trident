...
from trident_demo.benchmark.resource_monitor import ResourceTracker
...
class PerformanceRecorder:
    ...
    def start_wall(self) -> None:
        if not self.enabled:
            return
        if self._wall_start is not None and not self._wall_finished:
            return
        self._wall_start = perf_counter()
        self._wall_finished = False
        self.resources.start_run()

    def finish_wall(self) -> None:
        if not self.enabled or self._wall_start is None or self._wall_finished:
            return
        self.seconds["wall_clock_total"] = perf_counter() - self._wall_start
        self.resources.finish_run()
        self._wall_finished = True
