...
from trident_demo.pipeline.experiment import preprocess_features
from trident_demo.lib.utils import is_benign_label, split_year_label
...
def _rates_from_cv_confusion(report: Dict[str, Any]) -> Dict[str, Optional[float]]:
    cm = report.get("cv_confusion_matrix")
    if not cm:
        return {}
    labels = [str(x) for x in cm.get("labels", [])]
    matrix = cm.get("matrix")
    if not labels or not matrix or "0" not in labels or "1" not in labels:
        return {}
    i0, i1 = labels.index("0"), labels.index("1")
    tn, fp = int(matrix[i0][i0]), int(matrix[i0][i1])
    fn, tp = int(matrix[i1][i0]), int(matrix[i1][i1])
    n_benign, n_attack = tn + fp, tp + fn
    if n_benign == 0 or n_attack == 0:
        return {}
    return {
        "cv_fpr": float(fp / n_benign),
        "cv_fnr": float(fn / n_attack),
        "cv_tpr": float(tp / n_attack),
        "cv_tnr": float(tn / n_benign),
    }


def build_visualization_payload(reports: List[Dict[str, Any]]) -> Dict[str, Any]:
    tasks: List[Dict[str, Any]] = []
    for r in reports:
        task_name = str(r.get("task", ""))
        if task_name.startswith("learner"):
            scope = "learner"
        elif task_name.startswith("label"):
            scope = "label"
        else:
            scope = "flow"
        entry: Dict[str, Any] = {
            ...
            "class_distribution": r.get("class_distribution"),
            "cv_folds": r.get("cv_folds"),
            "cv_accuracy_mean": r.get("cv_accuracy_mean"),
            "cv_f1_macro_mean": r.get("cv_f1_macro_mean"),
            "cv_roc_auc": r.get("cv_roc_auc"),
            "train_accuracy": r.get("train_accuracy"),
            "top_feature_importance": r.get("top_feature_importance", [])[:20],
            "tree_rules": r.get("tree_rules", ""),
        }
        entry.update(_rates_from_cv_confusion(r))
        tasks.append(entry)
    return {"tasks": tasks}


def run_pipeline_decision_tree_analysis(
    output_dir: Path,
    ...
    *,
    ...
    """Run learner + dataset-label decision trees; write JSON for the visualize UI."""
    dt_cfg = cfg.get("decision_tree") or {}
    if not dt_cfg.get("enabled", True):
        return None

    output_dir = Path(output_dir)
    seed = int(cfg.get("runtime", {}).get("seed", 42))
    max_depth = int(dt_cfg.get("max_depth", 6))
    min_samples_leaf = int(dt_cfg.get("min_samples_leaf", 2))
    attack_threshold = float(dt_cfg.get("attack_threshold", 0.5))
    min_label_count = int(dt_cfg.get("min_label_count", 5000))

    learner_work = learner_df.copy()
    risk_path = output_dir / "learner_risk_scores.csv"
    if risk_path.exists():
        risk = pd.read_csv(risk_path)
        extra = [c for c in risk.columns if c not in learner_work.columns and c != "learner_name"]
        if extra:
            learner_work = learner_work.merge(risk[["learner_name"] + extra], on="learner_name", how="left")

    reports: List[Dict[str, Any]] = []
    if len(learner_work) >= 4:
        reports.extend(
            analyze_learner_level(
                learner_work,
    ...
    if not reports:
        if logger:
            logger.warning("Decision tree analysis skipped: insufficient rows.")
        return None

    summary_path = output_dir / "decision_tree_summary.json"
    viz_path = output_dir / "decision_tree_visualization.json"
    summary_path.write_text(json.dumps(reports, ensure_ascii=False, indent=2), encoding="utf-8")
    viz_path.write_text(
        json.dumps(build_visualization_payload(reports), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    for i, r in enumerate(reports):
        rules_path = output_dir / f"tree_rules_{i:02d}_{r['task'].replace('/', '_')[:60]}.txt"
        rules_path.write_text(str(r.get("tree_rules", "")), encoding="utf-8")
    if logger:
        logger.info("Done. DECISION_TREE_SUMMARY=%s (%d tasks)", summary_path, len(reports))
        logger.info("Done. DECISION_TREE_VIZ=%s", viz_path)
    return summary_path


def analyze_label_level(
    label_source: Path | pd.DataFrame,
    *,
    max_depth: int,
    min_samples_leaf: int,
    random_state: int,
    ...
    df = pd.read_csv(label_source) if isinstance(label_source, Path) else label_source.copy()
    numeric_cols = _numeric_feature_cols(df)
    reports: List[Dict[str, Any]] = []
