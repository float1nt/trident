...
const TRIDENT_API_PREFIX = "/trident-api";

function tridentRunDataApiPlugin(): Plugin {
  return {
    name: "trident-run-data-api",
    configureServer(server) {
      ...
      const isCompleteRun = (runId: string) => {
        ...
        return (
          summary.startsWith(runsRoot) &&
          overlap.startsWith(runsRoot) &&
          fs.existsSync(summary) &&
          fs.existsSync(overlap)
        );
      };
      ...
      server.middlewares.use(`${TRIDENT_API_PREFIX}/runs`, (_req: IncomingMessage, res: ServerResponse) => {
      ...
      server.middlewares.use(`${TRIDENT_API_PREFIX}/run-data`, (req: IncomingMessage, res: ServerResponse) => {
...
export default defineConfig({
  plugins: [react(), tridentRunDataApiPlugin()],
  resolve: {
    alias: {
