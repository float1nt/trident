...
import { getMockIpRiskEventTopology } from "@/mock/eventLearnerTopology";
import {
  getMockIpRiskEvents,
  getMockIpSummary,
  getMockIpTrafficLogs,
...
export default function IpDetailPlaceholder() {
  ...
  const summary = ip ? getMockIpSummary(ip) : undefined;
  const [riskEventPage, setRiskEventPage] = useState(1);
  const [trafficLogPage, setTrafficLogPage] = useState(1);

  const ipRiskEventTopology = useMemo(
    () => (ip ? getMockIpRiskEventTopology(ip) : null),
    [ip],
  );

  const riskEvents = useMemo(
    () => (ip ? getMockIpRiskEvents(ip) : []),
    [ip],
  );
  const trafficLogs = useMemo(
    () => (ip ? getMockIpTrafficLogs(ip) : []),
    [ip],
  );
